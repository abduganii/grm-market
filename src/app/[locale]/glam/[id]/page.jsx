...
async function getProduct({search,productId}) {
...
export default async function GilamById({ params, searchParams }) {
  ...
  const product = await getProduct({search:searchParams?.modelId + " " + searchParams?.color + " " + searchParams?.collectionId,productId:id});
